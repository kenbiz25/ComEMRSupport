...
composer = RagComposer(
    ...
    # Do NOT pass unsupported kwargs here.
)
...
async def inbound(req: Request):
    ...
    # Compose answer using RAG — pass session_id to enable conversation memory
    try:
        answer, meta = composer.answer(text_body, session_id=from_number)
    except Exception as e:
        ...
        meta = {"confidence": 0.0, "strategy": "error"}

    # Route-level confidence fallback
    confidence = float(meta.get("confidence", 0.0) or 0.0)
    strategy = (meta.get("strategy") or "").lower()
    if (
        getattr(settings, "ANSWER_CONFIDENCE_THRESHOLD", None) is not None
        and confidence < float(settings.ANSWER_CONFIDENCE_THRESHOLD)
        and strategy not in ("low_conf_threshold", "reject")
    ):
        answer = (
    ...
    try:
        ...
        log.info(f"Reply sent to {from_number} | confidence={confidence:.2f} | strategy={strategy}")
    except Exception as e:
        log.error(f"Failed to send WhatsApp message: {e}")
        return {"status": "send_error", "confidence": confidence, "strategy": strategy}

    return {"status": "sent", "confidence": confidence, "strategy": strategy}
